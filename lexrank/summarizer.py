...
class LexRank:
    ...
    def __init__(
        ...
        multi_document_summarization='single',
        corpus_size='max',
        cur_time=None,
        desc_title=None,
        desc_id=None,
        ...
        self.multi_document_summarization = multi_document_summarization    # 'single' or 'multi'
    ...
    def _calculate_average_word_contribute(self, tf_scores, i):
        ...
        for cur_word, word_amount in sentence_dict.iteritems():
    ...
    def _calculate_idf(self, documents):
        ...
        self.vectorizer = CountVectorizer(binary=True,
                                          ...
                                          vocabulary=None,
                                          lowercase=True)
        if self.corpus_size != 'max':       # corpus size limitation is a int value
            if self.corpus_size < len(documents):
        ...
        for word_idx, word in enumerate(dict_word_index):
            ...
            if doc_number_word > 0:
                idf_score[word] = math.log(float(doc_number_total) / float(doc_number_word))
            else:
                idf_score[word] = 0
    ...
    def _markov_matrix_discrete(self, similarity_matrix, threshold):
        ...
        for i in range(len(similarity_matrix)):
            ...
            markov_matrix[i, columns] = 1.0 / len(columns)      # changed 1 -> 1.0 (to be not all zero)
    ...
    def write_html_header(self, myFile):
        ...
        myFile.write('<p><b>Summarization version: </b>' + str(self.summarization_version) + '</p>')
        ...
        # header according to summarization method
    ...
    def create_file_path(self):
        ...
        html_dir_path = self.html_dir + str(self.cur_time) + '_LexRank_version=' + str(self.lex_rank_algorithm_version) \
                        + '_summarization=' + str(self.summarization_version)

        if self.lex_rank_algorithm_version == 'personality-based-LexRank':
            personality_str = ''
            for trait, value in self.user_personality_dict.iteritems():
        ...
        if self.summarization_version == 'top_relevant':
            html_dir_path = html_dir_path + '_summarization_max_similarity=' + \
                            str(self.summarization_similarity_threshold)

        html_dir_path = html_dir_path + '_doc=' + str(self.multi_document_summarization)
